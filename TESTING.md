...
| **Valid form submission - 3D Authentication**                 | Successful form submission using the Stripe Authenticate card number brings up the Stripe authentication popup.                  | Submit order with the Stripe authentication card number                                                              | PASS            |
| **Valid form submission - 3D Authentication**                 | Clicking ‘complete’ on authentication popup puts the order and payment through successfully                                      | Submit order with the Stripe authentication card number, click on ‘complete authentication’. Check order appears on order history, payment has gone through on Stripe and order appears on database                                                              | PASS            |
| **Valid form submission - 3D Authentication**                 | Clicking cancel or fail on authentication returns user to form with details filled in and an error message. Order is not created. | Submit order with the Stripe authentication card number, click on ‘fail authentication’, repeat with ‘cancel’ button. Check no order or payment has appeared on order history, Stripe payments and order on database.                                                              | PASS            |
